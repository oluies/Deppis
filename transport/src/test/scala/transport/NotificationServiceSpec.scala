...
class NotificationServiceSpec extends AnyFunSuite:
  ...
  private def digestOf(b: ByteString): Notification.Digest =
    ...
    (0 until bytes.length * 8).foreach(i => if (bytes(i >> 3) & (1 << (i & 7))) != 0 then d = d.set(i))
    d

  private def withClient(ns: DevNotificationServer)(body: NotificationServiceGrpc.NotificationServiceBlockingStub => Unit): Unit =
  ...
  test("signal then fetchDigest over gRPC reports the buddy's bit; fetch consumes it"):
    ...
    withClient(ns) { client =>
      ...
      val d2 = digestOf(client.fetchDigest(FetchDigestRequest(roundId = 1L, clientLabel = bs(label))).digest)
      assert(d2.isEmpty)
    }
  ...
  test("a forged token yields a uniform SignalResponse but never sets a bit (FR-003)"):
    val ns = DevNotificationServer(key)
    withClient(ns) { client =>
      val resp = client.signal(SignalRequest(roundId = 2L, sealedToken = bs(Array.fill[Byte](40)(0))))
      assert(resp.roundId == 2L) // uniform response, no error leaked
      val d = digestOf(client.fetchDigest(FetchDigestRequest(roundId = 2L, clientLabel = bs(label))).digest)
      assert(d.isEmpty)
    }
