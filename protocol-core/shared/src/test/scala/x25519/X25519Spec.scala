...
/** X25519 ECDH known-answer test (RFC 7748 §6.1). The SAME vectors run on the JVM (JCA) here and under
  ...
  * contract the double ratchet (post-compromise security) depends on. The peer-key REJECTION parity
  * (small-order / non-canonical keys) is single-sourced in `x25519.X25519RejectionCrossSpec`
  * (crosstest/, compiled into both builds), not duplicated here. */
