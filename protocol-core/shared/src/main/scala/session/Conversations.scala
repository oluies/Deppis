...
/** Per-buddy conversation state (T034, FR-006). Each buddy has its own independent send queue
  ...
  * or perturb another — the user can hold many conversations at once. The monotone counter feeds
  * retrieval-token derivation, keeping tokens non-recurrent per buddy (FR-014).
  *
  * Immutable: every operation returns a new `Conversations`. */
final class Conversations private (private val convs: Map[String, ConversationState]):

  def register(pairId: String, pairKey: Array[Byte]): Conversations =
    new Conversations(convs.updated(pairId, ConversationState(pairKey, 0L, Vector.empty)))
    ...
    convs.get(pairId) match
      ...
      case Some(st) =>
        Frame.pad(st.queue.head).map { fr =>
          ...
          val token   = RetrievalToken.derive(st.pairKey, pairId, pairId, counter)
