...
/** ChaCha20-Poly1305 AEAD (Scala.js, @noble/ciphers), under Node. Round-trip + the SAME static KAT
  * vector the JVM JCA build produces — proving noble ≡ JCA byte-for-byte, so the cross-compiled
...
class AeadJsSpec extends AnyFunSuite:
  ...
  test("seal/open round-trips on JS"):
    val ct = Aead.seal(key, nonce, "hello aead".getBytes("UTF-8"))
