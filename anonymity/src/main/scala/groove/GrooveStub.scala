...
final class GrooveStub extends AnonymityLayer:
  ...
  private val rounds = new ConcurrentHashMap[Long, java.util.List[Array[Byte]]]()
  ...
  def submit(roundId: Long, frame: Array[Byte]): Either[String, Unit] =
    rounds
      .computeIfAbsent(roundId, _ => java.util.Collections.synchronizedList(new java.util.ArrayList[Array[Byte]]()))
      .add(frame.clone())
    Right(())

  /** Single-shuffler: Fisher–Yates permute the round's frames and return up to `count`. The
    * permutation hides submission order within a round but is not a mix and adds no DP noise. */
