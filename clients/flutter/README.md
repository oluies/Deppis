...
#    sbt 2.0 puts link output under target/out/sjs1/scala-<version>/…, so let the shell resolve the
#    version rather than typing it — it changes whenever build.sbt's scalaVersion does.
...
# (or just: scripts/build-web-bundle.sh, which does both steps, checks the glob is unambiguous,
