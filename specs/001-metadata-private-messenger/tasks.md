...
- [~] T040 [P] [US6] Statistical indistinguishability test (active vs idle traces) in `server/src/test/scala/integration/cover/` (write first, MUST fail) — METADATA TRACE DONE: `RoundTransportSpec` asserts active and idle engines produce an identical **store-WRITE** trace AND an identical **FETCH** trace (one write + one read per round, fixed 256-byte frame, fixed 32-byte tokens). Remaining distinguisher: frame **content** (real plaintext vs all-zero carrier), closed once the message-content ratchet is in the frame path (T042).
- [X] T041 [US6] Implement uniform per-round carrier frames on send AND fetch paths (shape independent of real-message presence) in `protocol-core/shared/src/main/scala/schedule/` + `server/src/main/scala/round/` (FR-012) — `Engine.tick` makes exactly one store WRITE per round (real frame under its directional token if queued, else a carrier under a fresh `random.Rand`-derived cover token) AND exactly one READ per round. The read is **notify-guided** (FR-004): a *real* read happens only when the backend signals mail this round (round-robin over confirmed buddies), otherwise a fresh cover read — so a real expected token is read at most once and its counter advances, making the per-round read-token stream **non-recurrent** (FR-014) whether the client is active or idle. (Holds when notify is accurate — fires iff a message is present — which the PING/PONG path guarantees.) So neither the write nor the fetch trace, by count OR token recurrence, reveals whether a real message is present. Cost: receive latency is round-robin over buddies (cover-traffic tradeoff; per-buddy-digit-guided retrieval is a refinement). Platform-split CSPRNG (JVM `SecureRandom` / JS Web-Crypto `getRandomValues`). `recvCursor` is `Long` with `floorMod`. Tests: one-write+one-read per round, **non-recurrent token stream** active-or-idle, one-per-round delivery, real-obsd e2e.
