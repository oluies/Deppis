...
- [~] T040 [P] [US6] Statistical indistinguishability test (active vs idle traces) in `server/src/test/scala/integration/cover/` (write first, MUST fail) — DONE (client cover traffic, FR-012): `Engine.tick` now makes **exactly one store write every round** — a real frame under its outgoing token if queued, else a **carrier** frame under a fresh random-looking cover token (derived from a per-session `random.Rand` key; platform-split JVM `SecureRandom` / JS Web-Crypto `getRandomValues`, both sync). `RoundTransportSpec` asserts active and idle engines produce an **identical observable submit trace** (one write/round, fixed 256-byte frame, fixed 32-byte token) — an observer can't tell active from idle by traffic shape. **PENDING:** frame *content* uniformity (real plaintext vs all-zero carrier) needs the message ratchet in the frame path — orthogonal confidentiality layer; and store-side capacity/eviction of cover frames under sustained load is the Phase-C oblivious-store concern.
