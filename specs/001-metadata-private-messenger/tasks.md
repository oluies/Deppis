...
- [ ] T054 Implement the enclave-target `ObliviousStore`/notification fronts in `server/{pong,ping}/src/main/scala/.../enclave/` calling the sidecar over gRPC (Constitution VIII) — PARTIAL: `EnclaveObliviousStore` (transport) implements the ObliviousStore trait over the `store.proto` gRPC contract (private only when attested), tested in-process against a StoreService-backed dev store. Pending: the notification enclave front (over the oblivious aggregation) and exposing the Rust sidecar itself via tonic (so it's the real native sidecar, not a Scala shim)
