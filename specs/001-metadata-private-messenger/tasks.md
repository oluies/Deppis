...
- [X] T012 [P] Wrap an audited Signal double-ratchet implementation in `crypto/jvm/src/main/scala/ratchet/` (no hand-rolled ratchet — Constitution I) — wraps the audited `org.whispersystems:signal-protocol-java` (pinned 2.8.1) in `crypto/src/main/scala/ratchet/Ratchet.scala` (`RatchetParty`: publish X3DH bundle, establish session, encrypt/decrypt; never reimplements the ratchet). `RatchetSpec` (7): round-trip both ways, ratchet advances (same plaintext ⇒ different ciphertext), out-of-order delivery decrypts, PREKEY→WHISPER type transition, tampered-ciphertext rejected (auth failure surfaced), unknown-type rejected. **Tracked follow-up (T012a):** `signal-protocol-java` is archived/EOL — migrate to the maintained `org.signal:libsignal-client` (Rust core + Java bindings); the thin `RatchetParty` wrapper is the only coupling point, so the migration is localized. Kept pure-JVM for now (no native libs in CI).
...
- [~] T041 [US6] Implement uniform per-round carrier frames on send AND fetch paths (shape independent of real-message presence) in `protocol-core/shared/src/main/scala/schedule/` + `server/src/main/scala/round/` (FR-012) — SEND path DONE + SINGLE-BUDDY fetch DONE: `Engine.tick` makes exactly one store WRITE per round (real frame under its directional token if queued, else a carrier under a fresh `random.Rand`-derived cover token) AND exactly one READ per round. The read is **notify-guided** (FR-004): a *real* read happens only when the backend signals mail this round, otherwise a fresh cover read — so for a SINGLE buddy a real token is read at most once and the per-round read-token stream is **non-recurrent** (FR-014) whether active or idle. Platform-split CSPRNG (JVM `SecureRandom` / JS Web-Crypto `getRandomValues`); `recvCursor` `Long` with `floorMod`. Tests: one-write+one-read per round, single-buddy non-recurrence active-or-idle, one-per-round delivery, real-obsd e2e. **PENDING (T041b):** MULTI-BUDDY token non-recurrence — the per-client notify + round-robin read can land on a buddy with no mail and re-read its frozen token (recurrence); needs per-buddy notify (pinned by a `MULTI-BUDDY ... NOT yet held` test).
- [ ] T041b [US6] Per-buddy notify so multi-buddy fetch is token-non-recurrent (FR-012/FR-014): the engine signals the notify under the recipient's **per-buddy** label on send, and the receiver reads exactly the buddy whose signal is set (always a hit ⇒ always a fresh token), closing the multi-buddy recurrence + the worst-case receive-latency gate left by T041. Needs a `RoundTransport.signal` + per-buddy notify labels derived from the pair key.
