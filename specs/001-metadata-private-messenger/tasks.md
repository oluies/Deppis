...
- [X] T041 [US6] Implement uniform per-round carrier frames on send AND fetch paths (shape independent of real-message presence) in `protocol-core/shared/src/main/scala/schedule/` + `server/src/main/scala/round/` (FR-012) — `Engine.tick` makes exactly one store WRITE per round (real frame under its directional token if queued, else a carrier under a fresh `random.Rand`-derived cover token) AND exactly one READ per round (round-robin over confirmed buddies; a real expected token, or a cover read when none) — the schedule's one-send/one-retrieve invariant. So neither the write nor the fetch trace reveals whether a real message is present. Cost: receive latency is round-robin over buddies (the cover-traffic tradeoff; a digest-guided scheme with fixed padding is a refinement). Platform-split CSPRNG (JVM `SecureRandom` / JS Web-Crypto `getRandomValues`). Tests: one-write + one-read per round active-or-idle, one-per-round delivery, real-obsd e2e.
