...
- [X] T032 [US2] Wire `sendMessage` (frame+enqueue) and the `notified` engine event in `protocol-core/js/` + Flutter notification indicator in `clients/flutter/lib/notify/` (FR-004) — Flutter side (conversation send + `notified` indicator in `HomeScreen` that never names the buddy, + error banner, via `AppState`); engine-side `sendMessage`/`tick` in `engine.Engine` (T019); and the Flutter client now drives the **real** engine via `ScalaJsEngine` (the JSON boundary), selected by `createEngine()` on web (DevEngine fallback). `ScalaJsEngine` adapter unit-tested with a fake handle (6 tests); web binding via `dart:js_interop` to the linked bundle.
