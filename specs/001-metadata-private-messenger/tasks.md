...
- [~] T032 [US2] Wire `sendMessage` (frame+enqueue) and the `notified` engine event in `protocol-core/js/` + Flutter notification indicator in `clients/flutter/lib/notify/` (FR-004) — DONE: Flutter UI (conversation send + `notified` indicator in `HomeScreen` that never names the buddy, + error banner, via `AppState`); engine-side `sendMessage`/`tick` in `engine.Engine`; the Flutter client drives the **real** engine via `ScalaJsEngine` over the JSON boundary, selected by `createEngine()` on web (guarded DevEngine fallback); `ScalaJsEngine` translates `notified`/`messageReceived`/`privacyStatus`/`buddyConfirmed` events (adapter unit-tested, fake handle). **PENDING (T032a):** the real `engine.Engine` does not yet EMIT `notified`/`messageReceived` — those originate from the PING/PONG notify+store backend (`EnclaveNotificationClient`/`EnclaveObliviousStore`), which is not yet wired into the Scala.js engine. So on the real web backend the mail indicator stays quiet until that integration lands; `DevEngine.tick→notified` is a dev-only stand-in.
