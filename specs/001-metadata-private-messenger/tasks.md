...
- [X] T041b [US6] Per-buddy notify so multi-buddy fetch is token-non-recurrent (FR-012/FR-014) — the PING digest carries one bit per buddy (512 bits, FR-015); `RoundTransport.fetchDigest` returns it and the engine reads EXACTLY the buddy whose bit is set this round (bit derived from the pair key via `notifyBit`). Because that buddy's message is present, the read is a hit and its counter advances, so the per-round read-token stream is **non-recurrent for any buddy count** — closing the round-robin recurrence T041 left open. `RoundTransport.mailWaiting:Boolean` → `fetchDigest:Array[Byte]` (GrpcRoundTransport → `EnclaveNotificationClient.fetchDigest`; JsTransport facade updated). Tests: multi-buddy non-recurrence (read only the signaled buddy, A's frozen token never read), one-per-round delivery, real-obsd e2e signals `notifyBit`.
- [X] T041c [US6] Collision-free notify (FR-014 unconditional) — **done via per-round bit rotation + ambiguity-cover**, not a static pairing-time lease (the lease-less, derive-from-shared-secret architecture has no pairing channel to carry a lease; design: `design/notify-bit-lease.md`). `NotifyDigest.bit(pairKey, roundId)` rotates the bit per round so collisions are transient; `Engine.tick` serves a buddy ONLY when its set bit is unambiguous that round (a guaranteed hit ⇒ no read-token recurrence), deferring an ambiguous (colliding) round to a fresh cover read (buddies re-signal next round). Closes the birthday-rate bit-collision recurrence; the simultaneous-many-senders case (one delivered/round, others re-signal) is the same fairness-cursor path. Tests: `engine.RecurrenceGapsSpec` (collision no longer recurs + still delivers), `AnonymitySpec` (now unconditional, no distinct-bit precondition), `RoundTransportSpec`/`JsRoundTransportSpec` round-aware.
- [X] T041d [US6] Retry-safe (round-derived) addressing + stop-and-wait ARQ — closes the last two FR-014 recurrence residuals (rejected-submit write-side, removed-peer read-side). Addressing tokens are now `dirToken(addrKey, dir, roundId)` (round-derived), so a rejected submit or a missed/false-notified serve never recurs a token — closed by construction. Reliability is restored by a stop-and-wait ARQ (`ArqFrame` inner block `[ackSeq][msgSeq][message]`; head held + retransmitted under fresh round tokens until acked; advance-on-ack; ack-only frames; msgSeq de-dup), since round-derived tokens drop the persistent-store reliability the counter scheme relied on. Each tick writes this round's token and reads the PREVIOUS round's writes (`readRound = roundId-1`, one-round latency, no write/read barrier). Stage 1 (#61) ARQ framing/ack carriage; Stage 2 the round-derived flip. Tests: `RecurrenceGapsSpec` flips #2/#3 to assert non-recurrence; `RoundTransportSpec` connected-backend ARQ flow (in-order, bidirectional, multi-buddy no-starvation, reject→retransmit→deliver); `ArqFrameSpec` dedup; real-obsd e2e. Design: `design/retry-safe-addressing.md`. Now SC-002/FR-014 hold **unconditionally** (no non-rejecting-store / stops-signaling-once-removed assumptions); the remaining metadata gap is the oblivious backend (attested enclave).
...
- [X] T053a [P] Which-buddy anonymity test (SC-002): with N buddies and one communicating, the store host cannot identify the buddy better than 1/N — `engine.AnonymitySpec` (cross-platform, in CI). Captures the store host's observable (the receiver's read-token trace) across all N "which buddy is active" worlds and asserts it is INDISTINGUISHABLE: same shape (one read/round, fixed token length), non-recurrent across every buddy+round (no clustering), key-dependent (same position, different buddy ⇒ different token), over a channel proven to still deliver. Identical observable ⇒ 1/N. The store adding no distinguisher is obsd's oblivious selftest (T050/T052); the notify host's anonymity is obsd's oblivious aggregation (T053); the real-backend path is proven by `EngineBackendE2ESpec`/`TwoPartyE2ESpec`.
...
- [~] T057 Reproducible enclave build + publish measurement to the append-only transparency log as CoRIM reference values (Constitution X). **PUBLISH-AND-PROVE HALF DONE (software, tested):** `attestation.TransparencyLog` (RFC 6962 Merkle log over JCA SHA-256: inclusion + consistency proofs, prover + verifier; `TransparencyLogSpec` round-trips every tree size 1..16 and rejects every forgery) + `attestation.ReferenceLog` (append-only log of `Measurement`s; derives the verifier's `ReferenceValues` only from logged entries; `ReferenceLogTrust.trusts` accepts a measurement only with an inclusion proof to a PINNED root — `ReferenceLogSpec`: logged⇒trusted, unlogged/wrong-root⇒not). **REMAINING (toolchain/hardware-gated):** the reproducible build producing the measurement, the live SGX `sgx_sign dump` extraction, the public checkpoint publishing, and the CoRIM/CBOR leaf encoding — all documented in `deploy/enclave/README.md`.
- [~] T058 Flip `metadataPrivate:true` ONLY when backend=enclave-target AND attestation passes; client submits sealed token only post-attestation; remove the dev label on real builds (FR-016/Constitution IV/IX) — **CLIENT-SIDE LABEL WIRED**: the attestation→privacy gate already exists server-side (`attestation.AttestationGate.provision` → `ProvisionedEnclave.attested`; the enclave fronts `EnclaveObliviousStore`/`EnclaveNotificationClient` derive `BuildPrivacyStatus(EnclaveTarget, attested)`). Now the `RoundTransport` seam carries `privacyStatus` and `engine.Engine.privacyStatus` surfaces it, so the CLIENT label is true end-to-end: local-only ⇒ dev; unattested enclave-target ⇒ dev; attested ⇒ `METADATA PRIVATE` (`RoundTransportSpec` T058 case). REMAINING: the client submitting its sealed token only post-attestation, and removing the dev label on a real attested build (needs the live attestation handshake + a real hardware-backed `DcapAttestationVerifier` run — hardware-gated).
- [X] T059 [P] Phase C threat model + trust assumptions written into `threat-model.md`: §2 trust assumptions (uncompromised enclave; single TEE vendor unless heterogeneous; trusted verifier + reference log), the out-of-scope list (side channels, rollback, power, transient-execution — §2/§5), per-adversary CAN/CANNOT split (§3, store/notify/network/provider/endpoint), and the SC-002 which-buddy property now cross-referenced to its machine-checked test (`engine.AnonymitySpec`).
