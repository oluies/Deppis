...
- [~] T032 [US2] Wire `sendMessage` (frame+enqueue) and the `notified` engine event in `protocol-core/js/` + Flutter notification indicator in `clients/flutter/lib/notify/` (FR-004) — Flutter side done (conversation send + `notified` indicator in `HomeScreen` that never names the buddy, + error banner, wired through `AppState`); engine-side `protocol-core/js` wiring pending T019
...
- [~] T036a [P] [US3] End-to-end latency test: a two-party exchange completes within minute-order (single-digit rounds); asserts the tuned round interval meets SC-001, in `server/src/test/scala/integration/latency/` (SC-001) — write first, MUST fail [analyze C2] — PARTIAL: the two-party exchange itself is proven end-to-end through the real Rust `obsd` in `transport/.../TwoPartyE2ESpec` (pairing → retrieval-token → framing → oblivious store → notify → retrieval → unframe; oblivious + single-use), CI `integration` job. The SC-001 latency/round-interval timing assertion is the remaining piece.
