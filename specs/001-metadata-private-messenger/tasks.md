...
- [X] T012 [P] Wrap an audited Signal double-ratchet implementation in `crypto/jvm/src/main/scala/ratchet/` (no hand-rolled ratchet — Constitution I) — wraps the audited `org.whispersystems:signal-protocol-java` (pinned 2.8.1) in `crypto/src/main/scala/ratchet/Ratchet.scala` (`RatchetParty`: publish X3DH bundle, establish session, encrypt/decrypt; never reimplements the ratchet). `RatchetSpec`: round-trip both ways, ratchet advances (same plaintext ⇒ different ciphertext), out-of-order delivery decrypts, PREKEY→WHISPER type transition — 5 tests green.
