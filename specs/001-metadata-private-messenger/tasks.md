...
- [X] T041c [US6] Collision-free notify (FR-014 unconditional) — **done via per-round bit rotation + ambiguity-cover**, not a static pairing-time lease (the lease-less, derive-from-shared-secret architecture has no pairing channel to carry a lease; design: `design/notify-bit-lease.md`). `NotifyDigest.bit(pairKey, roundId)` rotates the bit per round so collisions are transient; `Engine.tick` serves a buddy ONLY when its set bit is unambiguous that round (a guaranteed hit ⇒ no read-token recurrence), deferring an ambiguous (colliding) round to a fresh cover read (buddies re-signal next round). Closes the birthday-rate bit-collision recurrence; the simultaneous-many-senders case (one delivered/round, others re-signal) is the same fairness-cursor path. Tests: `engine.RecurrenceGapsSpec` (collision no longer recurs + still delivers), `AnonymitySpec` (now unconditional, no distinct-bit precondition), `RoundTransportSpec`/`JsRoundTransportSpec` round-aware. Remaining notify/addressing gap: rejected-submit recurrence (retry-safe addressing) — still open, pinned in `RecurrenceGapsSpec`.
