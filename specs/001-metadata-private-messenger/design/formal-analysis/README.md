...
| [`graphs/hijack-attack.svg`](graphs/hijack-attack.svg) | `ratchet-pq-epoch-hijack.spthy` :: `pq_post_compromise_security` — **falsified, 14 steps** | **The limit of the positive result** (§5.3). With A1 dropped, an adversary *active on the rekey exchange* **injects its own ciphertext**: it reads the **honest** epoch public key off the compromised channel (`Epoch_pub` → `d_0_sdec` → `coerce[!KU( pk(~ek) )]`) and builds `aenc(ss, pk(~ek))` with an `ss` **it chose**. It never supplies a KEM public key of its own — no second `~ek` exists in the trace — which makes the attack *cheaper* than key substitution. The confirm tag is present and verifies, and cannot help: it proves knowledge of `ss`, and the attacker picked that `ss`. |
